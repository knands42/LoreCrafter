...
CREATE TRIGGER update_users_email_verification_updated_at
    BEFORE UPDATE
    ON "users_email_verification"
    FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

-- Delete expired email verification token
CREATE OR REPLACE FUNCTION delete_expired_email_verifications() RETURNS void AS
$$
BEGIN
    DELETE
    FROM users_email_verification
...
SELECT cron.schedule(
               'delete_expired_email_verifications_job',
               '0 2 * * *', -- Every day at 2:00 AM
               $$SELECT delete_expired_email_verifications();$$
       );
